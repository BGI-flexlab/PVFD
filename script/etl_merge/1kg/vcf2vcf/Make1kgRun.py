...
def main():
    ...
    for chromo in data.keys():
        fo = open('/ifs4/ISDC_BD/xiaopeng2/src/DHGV/script/1kg/vcf2vcf/run1kg.vcf2json.' + chromo + '.mapred.sh', 'w');
        fo.write('#!/bin/bash' + "\n");
        ...
        fo.write('OUTPUT="/user/xiaopeng2/DHGV/vcf2vcf/1000Genomes/$NAME/output"' + "\n");
        fo.write('FILE="/ifs4/ISDC_BD/xiaopeng2/src/DHGV/etl/t1kg.vcf2vcf.py"' + "\n");
        fo.write('$HADOOP fs -rm -r -skipTrash $OUTPUT' + "\n");
        fo.write('time -p $HADOOP jar $STREAMING_JAR -D mapreduce.job.name="DHGV.1000Genomes.$NAME.vcf2vcf" -D stream.num.map.output.key.fields=2 -D mapreduce.job.reduces=149 -output $OUTPUT -mapper "/bin/cat" -reducer $FILE -file $FILE -input $INPUT 1>$0.o 2>$0.e' + "\n");
        fo.close();
        
        # if chromo != 'chrY':
        #     foRun.write("sh /ifs4/ISDC_BD/xiaopeng2/src/DHGV/script/run1kg.vcf2json." + chromo + '.mapred.sh' + "\n");
        foRun.write("sh /ifs4/ISDC_BD/xiaopeng2/src/DHGV/script/1kg/vcf2vcf/run1kg.vcf2json." + chromo + '.mapred.sh' + "\n");
